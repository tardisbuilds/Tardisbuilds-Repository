...
if ADDON.getSetting('skin.source') == "0":
    ...
    SKIN_PATH = xbmc.translatePath("special://profile/addon_data/script.tvguide.tardis/")
...
class TVGuide(xbmcgui.WindowXML):
    ...
    def __init__(self):
        ...
        f = xbmcvfs.File('special://profile/addon_data/script.tvguide.tardis/categories.ini','rb')
    ...
    def loadTVDBImages(self):
        file_name = 'special://profile/addon_data/script.tvguide.tardis/tvdb.pickle'
    ...
    def getControl(self, controlId):
        ...
        try:
            ...
            #xbmc.log("EXCEPTION: (script.tvguide.tardis) TVGuide.getControl %s" % detail, xbmc.LOGERROR)
    ...
    def close(self):
        if not self.isClosing:
            ...
            f = xbmcvfs.File('special://profile/addon_data/script.tvguide.tardis/tvdb.pickle','wb')
            try:
            ...
            file_name = 'special://profile/addon_data/script.tvguide.tardis/custom_stream_urls_autosave.ini'
    ...
    def play_catchup(self, program):
        file_name = 'special://profile/addon_data/script.tvguide.tardis/catchup.ini'
    ...
    def ChooseStreamAddon(self, result, channel):
        ...
        if url:
            if url.startswith('plugin://'):
                ...
                name = "Playlist"
                icon = xbmcaddon.Addon('script.tvguide.tardis').getAddonInfo('icon')
        stream = ""
        title = ""
    ...
    def onActionEPGMode(self, action):
        ...
        if action.getId() in COMMAND_ACTIONS["NEXT_DAY"]:
            ...
            if results:
                if len(results) > 1:
                    ...
                    if what > -1:
                        ...
                        xbmcgui.Dialog().notification("TV Guide - Tardis", "Couldn't find: %s" % title)
                else:
                    if selection == 0:
            ...
            if kodi < 16:
                dialog.ok('TV Guide - Tardis', 'Editing categories in Kodi %s is currently not supported.' % kodi)
            else:
                ...
                f = xbmcvfs.File('special://profile/addon_data/script.tvguide.tardis/categories.ini','wb')
            ...
            xbmc.log('[script.tvguide.tardis] Unhandled ActionId: ' + str(action.getId()), xbmc.LOGDEBUG)
    ...
    def onActionQuickEPGMode(self, action):
        ...
        if action.getId() in COMMAND_ACTIONS["LEFT"]:
            ...
            xbmc.log('[script.tvguide.tardis] quick epg Unhandled ActionId: ' + str(action.getId()), xbmc.LOGDEBUG)
    ...
    def programSearch(self):
        ...
        file_name = "special://profile/addon_data/script.tvguide.tardis/title_search.list"
        f = xbmcvfs.File(file_name,"rb")
    ...
    def descriptionSearch(self):
        d = xbmcgui.Dialog()
        title = ''
        file_name = "special://profile/addon_data/script.tvguide.tardis/synopsis_search.list"
        f = xbmcvfs.File(file_name,"rb")
        searches = sorted(f.read().splitlines())
        f.close()
        actions = ["New Search", "Remove Search"] + searches
    ...
    def categorySearch(self):
        d = xbmcgui.Dialog()
        f = xbmcvfs.File('special://profile/addon_data/script.tvguide.tardis/category_count.ini',"rb")
    ...
    def _showContextMenu(self, program):
        ...
        if buttonClicked == PopupMenu.C_POPUP_REMIND:
            ...
            if ADDON.getSetting('search.type') == 'MySearch':
                script = "special://home/addons/script.tvguide.tardis/search.py"
            ...
            if program.is_movie == "Movie":
                ...
                selection = xbmcgui.Dialog().select("Choose media type",["Search as Movie", "Search as TV Show", "Search as Either"])
            where = ["movie","tv","multi"]
            url = "https://api.themoviedb.org/3/search/%s?query=%s&api_key=95dce72e6061e61d331a1f73dc12313e&include_adult=false&page=1" % (where[selection],title)
            r = requests.get(url)
            data = json.loads(r.content)
            results = data.get('results')
            id = ''
    ...
    def clear_catchup(self):
        if not self.playing_catchup_channel:
            return
        self.playing_catchup_channel = False
        filename = 'special://profile/addon_data/script.tvguide.tardis/catchup_channel.list'
    ...
    def catchup(self,channel):
        ...
        f = xbmcvfs.File('special://profile/addon_data/script.tvguide.tardis/catchup_channel.list','wb')
        first_cmd = None
    ...
    def playWithChannel(self, channel, program = None):
        ...
        if url:
            ...
            if ffmpeg:
                ...
                script = "special://home/addons/script.tvguide.tardis/playwithchannel.py"
                xbmc.executebuiltin('RunScript(%s,%s,%s)' % (script,channel.id,timestamp))

            script = "special://profile/addon_data/script.tvguide.tardis/playwithchannel.py"
            if xbmcvfs.exists(script):
                xbmc.executebuiltin('RunScript(%s,%s,%s)' % (script,channel.id,timestamp))
    ...
    def stopWith(self):
        ...
        if ffmpeg:
            folder = ADDON.getSetting('autoplaywiths.folder')
            script = "special://home/addons/script.tvguide.tardis/stopwithchannel.py"
            xbmc.executebuiltin('RunScript(%s)' % (script))

        script = "special://profile/addon_data/script.tvguide.tardis/stopwithchannel.py"
        if xbmcvfs.exists(script):
            xbmc.executebuiltin('RunScript(%s)' % (script))
        xbmc.Player().stop()
    ...
    def saveActions(self):
        file_name = 'special://profile/addon_data/script.tvguide.tardis/actions.json'
        f = xbmcvfs.File(file_name,"wb")
...
class StreamSetupDialog(xbmcgui.WindowXMLDialog):
        ...
        if controlId == self.C_STREAM_BROWSE_ADDONS:
            ...
            if stream:
                ...
                if len(lines) > 1:
                    ...
                    file_name = 'special://profile/addon_data/script.tvguide.tardis/addons.ini'
            ...
            if item:
                ...
                d = xbmcgui.Dialog()
                d.notification("TV Guide - Tardis", title, sound=False, time=500)
    ...
    def updateDirsInfo(self):
        file_name = 'special://profile/addon_data/script.tvguide.tardis/folders.list'
        f = xbmcvfs.File(file_name)
        lines = f.read().splitlines()
        folders = []
        for folder in lines:
    ...
    def addBrowseFolder(self):
        ...
        file_name = 'special://profile/addon_data/script.tvguide.tardis/icons.ini'
