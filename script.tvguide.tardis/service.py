...
class Service(object):
    ...
    def onInit(self, success):
        if success:
            xbmc.log("[script.tvguide.tardis] Background Update Starting...", xbmc.LOGNOTICE)
    ...
    def onCachesUpdated(self):
        ...
        xbmc.log("[script.tvguide.tardis] Background Update Finished", xbmc.LOGNOTICE)
        if ADDON.getSetting('background.notify') == 'true':
            d = xbmcgui.Dialog()
            d.notification("TV Guide - Tardis", "Finished Updating")
if __name__ == '__main__':
    ADDON = xbmcaddon.Addon('script.tvguide.tardis')

    version = ADDON.getAddonInfo('version')
    if ADDON.getSetting('version') != version:
        #text = xbmcvfs.File('special://home/addons/script.tvguide.tardis/changelog.txt','rb').read()
        #xbmcgui.Dialog().textviewer("TV Guide - Tardis",text)
    ...
    xbmcvfs.delete('special://profile/addon_data/script.tvguide.tardis/source.db-journal')
    lock = 'special://profile/addon_data/script.tvguide.tardis/db.lock'
    xbmcvfs.delete(lock)
    try:
        if ADDON.getSetting('autostart') == "true":
            xbmc.executebuiltin("RunAddon(script.tvguide.tardis)")

        if ADDON.getSetting('background.service') == 'true':
            monitor = xbmc.Monitor()
            xbmc.log("[script.tvguide.tardis] Background service started...", xbmc.LOGDEBUG)
            if ADDON.getSetting('background.startup') == 'true':
                ...
                if ADDON.getSetting('service.addon.folders') == "true":
                    xbmc.executebuiltin('RunScript(special://home/addons/script.tvguide.tardis/ReloadAddonFolders.py)')
            while not monitor.abortRequested():
                if ADDON.getSetting('service.type') == '0':
                    ...
                    xbmc.log("[script.tvguide.tardis] Service waiting for interval %s" % waitTime, xbmc.LOGDEBUG)
                else:
                ...
                if timeLeft < 0:
                    timeLeft = 0
                xbmc.log("[script.tvguide.tardis] Service waiting for %d seconds" % timeLeft, xbmc.LOGDEBUG)
                if timeLeft and monitor.waitForAbort(timeLeft):
                    break
                xbmc.log("[script.tvguide.tardis] Service now triggered...", xbmc.LOGDEBUG)
                Service()
                if ADDON.getSetting('service.addon.folders') == "true":
                    xbmc.executebuiltin('RunScript(special://home/addons/script.tvguide.tardis/ReloadAddonFolders.py)')
                now = time.time()
        ...
        xbmc.log('[script.tvguide.tardis] Uncaught exception in service.py: %s' % str(ex), xbmc.LOGDEBUG)
