# -*- coding: utf-8 -*-
#
#      Copyright (C) 2016 Wardoctor [wardoctor@tardisbuilds.com]
#
...
def select_countries():
    ...
    if result:
        ...
        countries = ','.join(ids)
        xbmcaddon.Addon(id = 'script.tvguide.tardis').setSetting('yo.countries',countries)


def select_providers():
    s = xbmcaddon.Addon(id = 'script.tvguide.tardis').getSetting('yo.countries')
    if not s:
        return
    countries = s.split(',')
    d = xbmcgui.Dialog()
    ...
    xbmcaddon.Addon(id = 'script.tvguide.tardis').setSetting('yo.%s.headend' % country, headend)
