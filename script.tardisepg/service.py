...
class MyMonitor(xbmc.Monitor):
    ...
    def tidySettings(self):
        files = []
        for i in range(10):
            enabled = tardis.GetSetting('INI_%d_E' % i) == 'true'
            if enabled:
                file = tardis.GetSetting('INI_%d' % i)
                files.append(file)
        ...
        for file in files:
            if len(file) > 0:
                tardis.SetSetting('INI_%d'   % index, file)
                tardis.SetSetting('INI_%d_E' % index, 'true')
                index += 1

        for i in range(index, 10):
            tardis.SetSetting('INI_%d'   % i, '')
            tardis.SetSetting('INI_%d_E' % index, 'false')
...
tardis.log ('Startup check for TTV plugins update')
xbmc.executebuiltin(cmd)


#legacy tidying up
dst = os.path.join(xbmc.translatePath('special://userdata/keymaps'), 'zTTV.xml')
