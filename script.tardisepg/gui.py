...
TTV_CHANNEL = ['Your Channel. Your content.', 'Your Channel. Your Choice.','All day, Every day...', 'Online Radio', 'Android only channel.']


ADDON      = tardis.ADDON
HOME       = tardis.HOME
SKIN       = tardis.SKIN
GMTOFFSET  = tardis.GetGMTOffset()
TRAILERS   = ADDON.getSetting('trailers.addon')
USTV       = ADDON.getSetting('ustv.addon')
IGNORESTRM = ADDON.getSetting('ignore.stream') == 'true'
META_IMAGE = tardis.GetSetting('meta.image')
...
class TVGuide(xbmcgui.WindowXML):
    ...
    def __init__(self):
        ...
        self.ignoreMissingControlIds = [C_CATEGORIES_CTRL, C_CATEGORIES_LIST, C_MAIN_PLUGIN_ICON, C_MAIN_PLUGIN_NAME]
        self.channelIdx = 0
        ...
        self.database = None
        self.categoriesList = tardis.getCategories()
    ...
    def getControl(self, controlId):
        try:
            ...
            if not self.isClosing:
                tardis.DialogOK(strings(SKIN_ERROR_LINE1), strings(SKIN_ERROR_LINE2), strings(SKIN_ERROR_LINE3))
                self.close()
            return None
    ...
    def showBlackout(self):
        return
        img = os.path.join(tardis.RESOURCES, 'blackout.jpg')
        self.hideBlackout()
    ...
    def onInit(self):
        ...
        xbmcgui.Window(10000).setProperty('TTV_WINDOW', str(windowID))
    ...
    def onActionEPGMode(self, action):
        ...
        if actionId in [ACTION_PARENT_DIR, KEY_NAV_BACK, ACTION_PREVIOUS_MENU]:
            if not confirmExit or tardis.DialogYesNo('Are you sure you wish to quit Tardis TV?'):
                self.close()
                return
        ...
        if actionId == ACTION_LEFT:
            ...
            if (program is not None) and (not tardis.isLimited()):
    ...
    def tryProgram(self, program):
        if self.playChannel(program.channel):
            return
        result = self.getstreamingService().detectStream(program.channel)
        if not result:
            if self.touch:
                return
    ...
    def _showContextMenu(self, program):
        ...
        if buttonClicked == PopupMenu.C_POPUP_REMIND:
            ...
            xbmc.executebuiltin('XBMC.RunAddon(plugin.video.iplayerwww)')

        elif buttonClicked == PopupMenu.C_POPUP_ITVPLAYER:
            xbmc.executebuiltin('XBMC.RunAddon(plugin.video.itv)')

        elif buttonClicked == PopupMenu.C_POPUP_TTVOOLS:
            self.refresh = True
            xbmc.executebuiltin('XBMC.RunAddon(script.tardisepg.tools)')
    ...
    def onFocus(self, controlId):
        ...
        imageSmall = os.path.join(tardis.RESOURCES, 'imageSmall.png')

        if not self.touch:
            if not DSF:
                if (program.imageSmall is None) or ('defaultimage' in program.imageSmall):
    ...
    def playChannel(self, channel):
        tardis.log('EPG Redraw: Sleep')
        while self.redrawingEPG:
            xbmc.sleep(300)
            tardis.log('EPG Redraw: Sleep ERROR')

        self.removeHighlight()
        control = self.getFocus()
        if control in [elem.control for elem in self.controlAndProgramList]:
            texture = os.path.join(tardis.RESOURCES, 'orange-focus.png')
    ...
    def iSearch(self, program):
        if tardis.DialogYesNo('Do you want to watch %s' % program.channel.title, 'Or catchup on %s' % program.title, noLabel='Live', yesLabel='Catchup'):
            xbmcgui.Window(10000).setProperty('TTV_CATCHUP', 'TRUE')
            tardis.removeKeymap()
            xbmcgui.Window(10000).clearProperty('TTV_CATCHUP')
    ...
    def prePlayOptions(self, channel):
        if not tardis.CATCHUP:
            tardis.log('_______ CATCH UP TURNED OFF _______')
            return False
        ...
        if not (catchup.isValid(program.channel.streamUrl)) or (catchup.getRecordURL(program.channel.title) is None):
            if inPast:
        ...
        # Should this become a set reminder eventually?
        if inFuture:
            if not program.channel.streamUrl.startswith('HDTV'):
                if tardis.DialogYesNo('Do you want to watch %s' % program.channel.title, 'Or set a recording for %s' % program.title, noLabel='Live', yesLabel='Record'):
    ...
    def catchup(self):
        try:
            ...
            tardis.log('------ Catch-up streamid ------')
            tardis.log(stream)

            HDTV   = (stream.startswith('HDTV')) or (stream.startswith('plugin://plugin.video.xl'))
            tardis.log(HDTV)

            if not catchup.isValid(stream):
                tardis.DialogOK('No catch-up service set up.', 'Reverting back to Live TV.')
                tardis.log('------ Catch-up NOT Valid ------')
                tardis.CloseBusy()
                return False
            ...
            if HDTV:
                start = program.startDate # - datetime.timedelta(minutes=60)
                try:
                    url   = catchup.getHDTVRecording(name, program.title, start, stream)
                    tardis.CloseBusy()
                except:
                    tardis.DialogOK('No catch-up stream found for: [COLOR orange][B]%s[/B][/COLOR]' % (program.title), '[COLOR orange]Reverting back to Live TV.[/COLOR]')
                    return False

            else:
                start = program.startDate

                isRecorded, url = catchup.isRecorded(name, start)
                programID = None

                if not isRecorded:
                    programID = catchup.record(name, start, end, stream, showResult=False)

                tardis.CloseBusy()

                if (not isRecorded) and (not programID):
                    tardis.DialogOK('Unable to play %s' % name, 'Catchup requires temporary DVR space on Filmon.', 'Please delete some recordings and try again.')
                    return True

                if not isRecorded:
                    url = catchup.getRecording(name, start)

            if not url:
                return False

            wasPlaying = self.player.isPlaying()
            xbmcgui.Window(10000).setProperty('TTV_CHANNEL', name)
            xbmcgui.Window(10000).setProperty('TTV_CATCHUP', 'TRUE')
            tardis.removeKeymap()
            path = os.path.join(ADDON.getAddonInfo('path'), 'player.py')
            xbmc.executebuiltin('XBMC.RunScript(%s,%s,%d)' % (path, url, False))
    ...
    def onRedrawEPG(self, channelStart, startTime, focusFunction = None):
        ...
        #if self.getIni and not DSF:
        #    tardis.log('-------- TTV UPDATE INI FILES --------')
    ...
    def onEPGLoadError(self):
        tardis.log('Delete DB Tardis TV - onEPGLoadError')
        xbmcgui.Dialog().ok(strings(LOAD_ERROR_TITLE), strings(LOAD_ERROR_LINE1), strings(LOAD_ERROR_LINE2), strings(LOAD_ERROR_LINE3))
        self.close()
        xbmc.executebuiltin('XBMC.RunScript(special://home/addons/script.tardisepg/deleteDB.py)')
        tardis.log('****** Delete DB complete *******')


    def onSourceNotConfigured(self):
        tardis.log('Delete DB Tardis TV - onSourceNotConfigured')
...
class PopupMenu(xbmcgui.WindowXMLDialog):
    ...
    def onInit(self):
        # self.getControl(self.C_POPUP_TTVOOLS).setVisible(False) RD -Temporary hide of the 4oD button until a new use is found for it.
...
class StreamSetupDialog(xbmcgui.WindowXMLDialog):
    ...
    def onInit(self):
        ...
        for id in self.streamingService.getAddons():
            try:
                ...
                if id == 'script.tardis.tv':
                    item = xbmcgui.ListItem('Kodi PVR', iconImage=os.path.join(tardis.RESOURCES, 'kodi-pvr.png'))
                    item.setProperty('addon_id', id)
                items.append(item)
            except Exception:
                pass
    ...
    def onClick(self, controlId):
        if controlId == self.C_STREAM_STRM_BROWSE:
            ...
            if chooser.GetFave('TTV'):
                path  = xbmc.getInfoLabel('Skin.String(TTV.Path)')
                label = xbmc.getInfoLabel('Skin.String(TTV.Label)')
...
class CategoriesMenu1(object):
    ...
    def show(self):
        try:
            if (tardis.isLimited()) or (tardis.GetSetting('hide.cats') == 'true'):
                return self.hide()
    ...
    def onClick(self, controlId):
        ...
        SINGLE_CAT_MODE = tardis.GetSetting('SINGLE_CAT') == 'true'
        ...
        if category in self.categories:
            ...
            if SINGLE_CAT_MODE:
                ...
                for idx, dummy in enumerate(self.allCategories):
                    listControl.getListItem(idx).setIconImage('tardis-category-hidden.png')
            else:
                self.categories.append(category)

        if category in self.categories:
            iconImage = 'tardis-category-visible.png'
        else:
            iconImage = 'tardis-category-hidden.png'

        item.setIconImage(iconImage)

        self.parent.categoriesList = self.categories
        tardis.SetSetting('categories', '|'.join(self.parent.categoriesList))
        self.onRedrawEPG()
    ...
    def verify(self, categories):
        ...
        self.allCategories = self.database.getCategoriesList()
        self.updateCategories(categories)
