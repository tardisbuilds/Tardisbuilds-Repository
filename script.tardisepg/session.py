...
def getUsername():
    return tardis.GetSetting('username')


def getPassword():
    return tardis.GetSetting('password')
...
baseurl    = tardis.GetLoginUrl()
datapath   = tardis.PROFILE
cookiepath = os.path.join(datapath, 'cookies')
cookiefile = os.path.join(cookiepath, 'tardis.lwp')
...
def checkFiles(url):
    ...
    tardis.log('----- Check Tardis TV Files -----')
    tardis.log('---------- status code ----------')
    tardis.Log(code)
