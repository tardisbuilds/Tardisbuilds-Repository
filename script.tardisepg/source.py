...
#OPEN_TTV  = tardis.OPEN_TTV
#CLOSE_TTV = tardis.CLOSE_TTV
...
channelFolder = tardis.GetChannelFolder()
channelPath   = os.path.join(channelFolder, 'channels')
tardis.log('Channel Folder Setting: %s' % channelPath)
...
def GetLogoType():
    return tardis.GetSetting('logo.type')
...
def GetLogoFolder():
    ...
    if logoType == CUSTOM:
        logoFolder = tardis.GetSetting('user.logo.folder')
    else:
        logoFolder = tardis.GetSetting('tardis.logo.folder')
...
class Database(object):
    ...
    def eventLoop(self):
        tardis.log('Database.eventLoop() >>>>>>>>>> starting...')
        while True:
            self.event.wait()
            self.event.clear()
            ...
            tardis.log('Database.eventLoop() >>>>>>>>>> processing command: %s' % command.__name__)
            ...
            try:
                ...
                tardis.log('Database.eventLoop() >>>>>>>>>> exception! %s' % str(e))
                raise

        tardis.log('Database.eventLoop() >>>>>>>>>> exiting...')
    ...
    def initializeChannels(self):
        ...
        if logoType == CUSTOM:
            ...
            logoPath = 'special://profile/addon_data/script.tardisepg/extras/logos/'

        tardis.log('Logo Path Setting:   %s' % logoPath)
        tardis.log('Logo Folder Setting: %s' % logoFolder)
        tardis.log('Logo Type:           %s' % logoType)

        prevLogoFolder = tardis.GetSetting('PREVLOGO')
        currLogoFolder = logoFolder

        if currLogoFolder == prevLogoFolder:
           return True

        tardis.SetSetting('PREVLOGO', currLogoFolder)
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    ...
                    tardis.log('EPG Database is locked, retrying...')

            except sqlite3.DatabaseError:
                self.connP = None
                if self.alreadyTriedUnlinking:
                    tardis.log('EPG Database is broken and unlink() failed')
                    break
                else:
                    try:
    ...
    def _wasSettingsChanged(self, addon):
        ...
        tardis.log('Settings changed: ' + str(settingsChanged))
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        update = tardis.GetSetting('updated.channels')
        if int(update) < 0:
            tardis.SetSetting('updated.channels', 0)
            tardis.SetSetting('current.channels', 0)
        else:
            tardis.SetSetting('current.channels', update)
            self.channelDict = {}
            self.updateInProgress = False
    ...
    def deleteOldChannels(self, toDelete):
        tardis.log('START deleteOldChannels')
        ...
        for i in xrange(len(toDelete), 0, -1):
            ...
            try:
                ...
                if channel.isClone == 1:
                    ...
                    tardis.log('Clone of %s' % original)
                    if original not in toDelete:
                        tardis.log('NOT removing clone %s' % id)
                        toDelete.remove(id)

                if channel.userDef == 1:
                    tardis.log('NOT removing userdef %s' % id)
                    toDelete.remove(id)

            except Exception, e:
                tardis.log('ERROR in deleteOldChannels %s' % str(e))

        tardis.log('Pass 2')
        tardis.log(toDelete)
        for id in toDelete:
            try:                
                tardis.log('Channel %s no longer available' % id)
                self.removeCleanChannel(id)
            except:
                pass

        tardis.log('END deleteOldChannels')
    ...
    def getAllChannels(self):
        ...
        try:
            ...
            tardis.log('Error in getAllChannels' % str(e))
            return channels
    ...
    def getChannelFromFile(self, id):
        ...
        ch = Channel(cfg[0], cfg[1], cfg[2], cfg[3], cfg[4], cfg[5], cfg[6], cfg[10])

        return ch


    def addToFavourites(self, channel):
        FAVOURITES = tardis.GetSetting('FAVOURITE')
        faveCat    = FAVOURITES + '|'

        channelFile  = self.getChannelFromFile(CleanFilename(channel.id))

        categories   = channelFile.categories
        isFavourite  = channelFile.isFave == 1

        if not isFavourite:
            added = faveCat + categories
            channel.categories = added
            channel.isFave = 1

        else:
            removed = categories.replace(faveCat, '')
            channel.categories = removed
            channel.isFave = 0
    ...
    def isValid(self, channel, isProtected, isSwiss):
        if (tardis.ADULT in channel.categories) and not isProtected:
            return False

        if (tardis.SWISS in channel.categories) and not isSwiss:
            return False
    ...
    def _getEPGView(self, channelStart, date, progress_callback, clearExistingProgramList, categories, nmrChannels):
        update = xbmcgui.Window(10000).getProperty('TTV_UPDATE')
        if len(update) > 0:
        ...
        isProtected = tardis.isProtected()
        isSwiss     = tardis.isSwiss()
    ...
    def _getCategoriesList(self):
        ...
        for category in categoriesList:
            if FAVOURITES not in categoriesList:
                categoriesList.insert(0, FAVOURITES)

        try:
            if not tardis.isProtected():
                categoriesList.remove(tardis.ADULT)
        except:
            pass

        try:
            if not tardis.isSwiss():
                categoriesList.remove(tardis.SWISS)
        except:
            pass

        try:
            categoriesList.remove('uncategorised')
        except: pass

        tardis.log('-------------- categories ---------------')
        tardis.log(categoriesList)
    ...
    def _getProgramList(self, channels, startTime): #TODO Notifications
        ...
        try:
            ...
            tardis.log('------------- Program List Error -------------')
            tardis.DialogOK('An Error has occured.', 'Too many keys/buttons pressed on start-up.', 'Please force close Tardis TV')
            return []
...
class TARDISSource(Source):
    ...
    def isUpdated(self, channelsLastUpdated, programsLastUpdated):
        ...
        current = int(tardis.GetSetting('current.channels'))
        update  = int(tardis.GetSetting('updated.channels'))

        #tardis.log('current = %d' % current)
        #tardis.log('update  = %d' % update)

        if update == -1:
            tardis.SetSetting('updated.channels', current)
            return True
    ...
    def getDataFromExternal(self, date, progress_callback = None):
        ...
        categories   = tardis.getCategoryList(categoryPath)
        ...
        try:
            ...
            tardis.log('Error reading chan.xml')
            ...
            if elem.tag == "programme":
                ...
                if categories:
                    try:
                        ...
                        # tardis.log('The category for %s is %s' % (title, category))
                    except:
                        tardis.log('Couldnt find %s in the categories' % (title))
