...
ADDON    = tardis.ADDON
LOCAL    = tardis.GetSetting('local.ini') == 'true'
FTVINI   = tardis.GetSetting('ftv.ini')
...
class StreamsService(object):
    ...
    def loadPlaylist(self):
        iptv_type = tardis.GetSetting('playlist.type')
        IPTV_URL  = '0'
        IPTV_FILE = '1'
        ...
        if iptv_type == IPTV_FILE:
            path = os.path.join(tardis.GetSetting('playlist.file'))
        else:
            url  = tardis.GetSetting('playlist.url')
    ...
    def detectStream(self, channel):
        ...
        for id in self.getAddons():
            ...
            for (label, stream) in self.getAddonStreams(id):
                if (id == 'plugin.video.meta') or (id == 'plugin.video.metalliq'):
                    label = channel.title
