...
class StreamAddonDialog(xbmcgui.WindowXMLDialog):
        ...
        for id, label, url in self.addons:
            try:
                if id.startswith('SF_'):
                    ...
                    icon = os.path.join(tardis.RESOURCES, 'kodi-favourite.png')
                    name = 'kodi-favourite'

                elif id == 'iptv-playlist':
                    icon = os.path.join(tardis.RESOURCES, 'iptv-playlist.png')
                    name = 'iptv-playlist'

                elif id == 'script.tardis.tv':
                    icon = os.path.join(tardis.RESOURCES, 'kodi-pvr.png')
                    name = 'kodi-pvr'
    ...
    def onClick(self, controlId):
        if controlId == StreamAddonDialog.C_SELECTION_LIST:
            ...
            self.label  = listControl.getSelectedItem().getProperty('channelLabel')
            self.close()
