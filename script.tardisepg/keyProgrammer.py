...
def main(filename, cmd, setting, text):
    key = recordKey(text)
    if key < 1:
        tardis.DialogOK('Tardis TV Hot Key assignment cancelled')
        return

    start = 'key id="%d"' % key
    end   = 'key'

    if tardis.WriteKeymap(start, end, filename, cmd):
        tardis.log('TTV %s HotKey assigned: %s' % (text, key))
        if len(setting) > 0:
            tardis.SetSetting(setting, key)
        tardis.DialogOK('Tardis TV Hot Key successfully set up.', '', 'Thank you.')
