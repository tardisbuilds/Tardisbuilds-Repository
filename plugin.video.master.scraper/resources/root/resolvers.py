...
def resolve(url):
		...
		if 'tvcatchup' in url:
			...
			if '404 Not Found' in open:
				open = requests.session().get('http://173.212.202.101/token5004.php',headers=headers).text
			link = url+open
			...
			try:
				...
				headers = {'Authorization': 'Basic QFN3aWZ0MTEjOkBTd2lmdDExIw==',
					'User-Agent': 'Dalvik/1.6.0 (Linux; U; Android 4.4.4; SM-G900F Build/KTU84Q)',
					'Accept-Encoding': 'gzip'}
				open = requests.session().get('http://173.212.202.101/token5001.php',headers=headers).text
				
			link = url+(open).replace('eMeeea/1.0.0.','')
			url  = link+'|User-Agent=user'
