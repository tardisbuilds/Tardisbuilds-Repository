...
addon_id   = 'plugin.video.master.scraper'
...
def cat():
	...
	addDir('[COLOR white][B]Snappy Streams[/COLOR][/B]','snappystreams',4,'https://i.ytimg.com/vi/SZ5BkBmHbFk/hqdefault.jpg',fanart,'')
...
def swiftstreamschans(url):
	...
	for a in js:
		...
		desc = a['channel_desc']
		addDir('[B][COLOR white]%s[/COLOR][/B]'%name,'swiftstreams:'+url,10,'http://swiftstreamz.com/SwiftStream/images/thumbs/' + icon,fanart,desc)
...
def geotv():
	...
	open = requests.get('http://163.172.211.23:25461/enigma2.php?username=pakis&password=chnlss&type=get_live_categories',headers={'User-Agent':'Dalvik/1.6.0 (Linux; U; Android 4.3.1; WT19M-FI Build/JLS36I)'}).text
...
def geotvchans(url):
	...
	for a in all_cats:
		...
		addDir(name,urllib.quote_plus(url1),9999,thumb,fanart,base64.b64decode(desc))
...
def mobdro():
        import re
        file = xbmc.translatePath('special://home/addons/plugin.video.master.scraper/resources/')
...
logfile    = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.master.scraper', 'log.txt'))
