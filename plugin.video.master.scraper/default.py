...
def home():
	addDir('UK Geo Locked','url',1000,icon,fanart,'')
	addDir('Web Scrapers','url',2000,icon,fanart,'')
	addDir('IPTV Scrapers','url',3000,icon,fanart,'')
	addDir('Android API','url',4000,icon,fanart,'')
...
def popupd(announce):
	...
	class TextBox():
		...
		def setControls(self):
			self.win.getControl(self.CONTROL_LABEL).setLabel('Master Scraper') # set heading
...
if mode==None or url==None or len(url)<1:
	...
	#try:
	#	msg = OPEN_URL('http://163.172.46.122/files/masterbeta/startupmsg.txt')
