...
def Main():
    if AUTOSTREAM:
        if xbmc.getCondVisibility('Player.HasVideo') <> 1:
            url = xbmcaddon.Addon('script.tardisepg').getSetting('streamURL')
            if len(url) > 0:
                # PlayMedia(url)
    ...
    AddAddon('TV Guide',   'script.tardisepg', _SCRIPT,  icon=os.path.join(IMAGES, 'TTV-TV_Guide.png'))

    ShowMovie()
    ShowTV()

    AddSubFolder('TV Catch Up', _CATCHUP,   icon=os.path.join(IMAGES, 'TTV-TV_Catchup.png'), desc='Catch up on shows you have missed')

    AddAddon('Search',          SUPERFAVES, _SUPERSEARCH, icon=os.path.join(IMAGES, 'TTV-Search.png'), desc='Search all your favourite addons all from one place')
    AddAddon('Favourites',      SUPERFAVES, _ADDON,       icon=os.path.join(IMAGES, 'TTV-Favourites.png'))

    AddSubFolder('Tools',       _TOOLS,     icon=os.path.join(IMAGES, 'TTV-Tools.png'),  desc='Settings and Tools')

    ShowShortcuts()
    ShowSFShortcuts(SFFILE, _REMOVESUPERFAVE)
    icon = os.path.join(IMAGES, 'TTV-Shortcuts.png')
...
def ShowMovie():
    ...
    if utils.getSetting('UseKodiLib') == 'true':
        AddLibrary('Movies', '', _MOVIE, icon=os.path.join(IMAGES, 'TTV-Movies.png'))

    elif (len(sfave) == 0) and (addon == ''):
        AddLibrary('Movies', '', _MOVIEOPTIONS, icon=os.path.join(IMAGES, 'TTV-Movies.png'))

    elif not addon == '':
        AddMovieLibrary('Movies', '', _MOVIEADDON, icon=os.path.join(IMAGES, 'TTV-Movies.png'))
...
def ShowTV():
    ...
    if utils.getSetting('UseKodiLib') == 'true':
        AddLibrary('TV Shows', '', _TV, icon=os.path.join(IMAGES, 'TTV-TV_Shows.png'))

    elif (len(sfave) == 0) and (addon == ''):
        AddLibrary('TV Shows', '', _TVOPTIONS, icon=os.path.join(IMAGES, 'TTV-TV_Shows.png'))

    elif not addon == '':
        AddTVLibrary('TV Shows', '', _TVADDON, icon=os.path.join(IMAGES, 'TTV-TV_Shows.png'))
...
def ShowCatchup():
    AddAddon('BBC iPlayer', 'plugin.video.iplayerwww', _ADDON, icon=os.path.join(IMAGES, 'TTV-BBC_iPlayer.png'))
    AddAddon('ITV Player',  'plugin.video.itv',        _ADDON, icon=os.path.join(IMAGES, 'TTV-ITV_Player.png'))
    AddAddon('UKTV Play',   'plugin.video.uktvplay',   _ADDON, icon=os.path.join(IMAGES, 'TTV-UKTV_Play.png'))


def ShowTools():
    AddAddon('Add Channel Line-up', TOOLS,  _LINEUPS,       icon=os.path.join(IMAGES, 'TTV-add-lineups.png'),     desc='Install customised channel line-ups created by the Tardis TV Community.')
    AddAddon('Channels',            TOOLS,  _CHANNELS,      icon=os.path.join(IMAGES, 'TTV-edit-channels.png'),   desc='Edit your channels - Change logos, change visibility, change order or even add your own!')
    AddAddon('Categories',          TOOLS,  _CATEGORIES2,   icon=os.path.join(IMAGES, 'TTV-edit-categories.png'), desc='Edit your categories - Change a category name or remove a category altogether.')
    AddAddon('Back-up + Restore',   TOOLS,  _BACKUPRESTORE, icon=os.path.join(IMAGES, 'TTV-backup-restore.png'),  desc='Make full back ups or just back-up your channel line-up.')
    AddAddon('Install Skins',       TOOLS,  _SKINS,         icon=os.path.join(IMAGES, 'TTV-install-skins.png'),   desc='Install new skins and use them to change the EPG look and feel.')
    AddAddon('Install Logo-Packs',  TOOLS,  _LOGOS,         icon=os.path.join(IMAGES, 'TTV-install-logos.png'),   desc='Install new logo-packs and use them in the EPG.')
    AddAddon('Update Add-on Links', TOOLS,  _INI,           icon=os.path.join(IMAGES, 'TTV-update-addons.png'),   desc='Update the built-in Add-on links to 3rd party live TV add-ons available for Kodi.')
    AddVPNicity()
...
def OpenTools():
    xbmc.executebuiltin('ActivateWindow(10025,"plugin://script.tardisepg.tools/",return)')
...
def AddSFShortcut(file):
    if not chooser.GetFave('TTV'):
        return False

    path   = xbmc.getInfoLabel('Skin.String(TTV.Path)')
    label  = xbmc.getInfoLabel('Skin.String(TTV.Label)')
    icon   = xbmc.getInfoLabel('Skin.String(TTV.Icon)')
    folder = xbmc.getInfoLabel('Skin.String(TTV.IsFolder)') == 'true'
...
def AddVPNicity():
    try:
        ...
        AddDir('Select VPN', addonID, _VPNICITY, iconimage=os.path.join(IMAGES, 'TTV-open-vpn.png'), description='Activate Open-VPN Connect and pick a country.', isFolder=False, isPlayable=False)
    except:
        pass
