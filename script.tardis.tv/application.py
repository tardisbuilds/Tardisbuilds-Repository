...
class Application(xbmcgui.WindowXML):
    ...
    def onTimer(self):
        ...
        if xbmcgui.Window(10000).getProperty('TTV_RELAUNCH') == 'true':
            xbmcgui.Window(10000).setProperty('TTV_RELAUNCH', 'false')
            if not FIRSTRUN:
                self.doRelaunch()
    ...
    def onAction(self, action):
        ...
        if id == UPDATE:
            line1 = 'Attention , clicking Yes , you update the new GVAX application'
            line2 = ''
            line3 = 'OK?'

            if utils.DialogYesNo(line1, line2, line3, 'No', 'Si'):
