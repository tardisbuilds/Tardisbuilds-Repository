...
#import common
addon_id='plugin.program.tardis.notifications' ##Change this to the same as the addon.xml
...
try: 				from addon.common.net import Net
    ...
    try: 		from t0mm0.common.net import Net###Modded by FTG###
...
def SettingS(setting,value):
	settings.setSetting(id=setting,value=value)###Modded by FTG###
...
def OPEN_URL(url):
  ...
  req.add_header('User-Agent','Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.9.0.3) Gecko/2008092417 Firefox/3.0.3')###Modded by FTG###
...
hubLogo=artp('logo-rectangular3'); ###Modded by FTG###
#hubLogo=artp('logo_notice.png'); 
DefaultNoteImage=artp('blank1'); 
DefaultSplitter="|||"; 
DefaultUrl="http://tardisbuilds.com/Builds/TardisNotifications.txt"; ##change to where your .txt file is on your host
class MyWindow(xbmcgui.WindowDialog): #xbmcgui.Window): ##xbmcgui.Window
	...
	def doDismiss(self):
		try:
			...
			SettingS("noteMessage",self.noteMessage); ###Modded by FTG###
		except: pass
		##CODE HERE##
		self.CloseWindow1st()
...
def FetchNews():
	...
	if os.path.isfile(info_location)==True: 
		...
		try: 
			info_location3B=File_Open(info_location3).strip()###Modded by FTG###
