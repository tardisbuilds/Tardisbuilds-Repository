...
AccLink = "http://sgstreams.tv:25461/panel_api.php?username=%s&password=%s"%(Username,Password,)
...
def MyAccDetails(url):
        ...
        for url in match:
                AddAccInfo('My Stargate Streams Account Information','','',Images +'MyAcc.png')
...
def wizard2(name,url,description):
    ...
    dp.create("Stargate Streams","Downloading ",'', 'Please Wait')
    ...
    dp = xbmcgui.Dialog()
    dp.ok("Stargate Streams", 'TV Guide Integration Complete - Now open your Tardis TV & Enjoy. If you have any trouble just get in touch in the Stargate Streams #Facebook group & We are happy to help.', '', '')
...
def ExtraMenu():
    ...
    for name,url,iconimage,FanArt,description in match:
        addXMLMenu(name,url,8,iconimage,FanArt,description)
def Movies():
    link = OPEN_URL('').replace('\n','').replace('\r','')  #Spaf
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    for name,url,iconimage,FanArt,description in match:
...
def TVShows():
    ...
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    for name,url,iconimage,FanArt,description in match:
        addXMLMenu(name,url,mode,iconimage,FanArt,description)
