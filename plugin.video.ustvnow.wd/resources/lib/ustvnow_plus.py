...
class Ustvnow:
	    ...
	    for i in results:
	        try:
	            if i['order'] == 1:
	                name = Addon.cleanChanName(i['stream_code'])
	                if name == get_name:
		                stream = self._get_json('stream/1/live/view', {'token': self.token, 'key': self.passkey, 'scode': i['scode']})['stream']
	                        if quality == 4:
		                    url = stream
                                else:
	    ...
	    for i in results:
	        try:
	            ...
	            if scheduleid == get_scheduleid:
		            stream = self._get_json('stream/1/dvr/play', {'token': self.token, 'key': self.passkey, 'scheduleid': i['scheduleid']})['stream']
    ...
    def get_sports(self, quality, type='now'):
	    ...
	    sports = ['Basketball', 'Football', 'Baseball', 'Soccer', 'Tennis', 'Golf', 'Skating', 'Skateboarding', 'Skiing', 'Snowboarding', 'Rugby', 'Nascar', 'Bowling', 'Olympics', 'Paralympics']
	    for i in results:
	        from datetime import datetime
	        event_time = datetime.fromtimestamp(i['ut_start']).strftime('%I:%M %p').lstrip('0')
	        event_date_month = datetime.fromtimestamp(i['ut_start']).strftime('%m').lstrip('0')
	        ...
	        try:
	            if type == 'now' and i['order'] == 1:
	                ...
	                rec_url = '/gtv/1/dvr/updatedvr?scheduleid=' + str(i['scheduleid']) + '&token=' + self.token + '&action=add'
	                set_url = '/gtv/1/dvr/updatedvrtimer?connectorid=' + str(i['connectorid']) + '&prgsvcid=' + str(i['prgsvcid']) + '&eventtime=' + str(i['event_time']) + '&token=' + self.token + '&action=add'
	                if any(i['title'].find(s)>=0 for s in sports) or name == 'ESPN' or name == 'ESPN2' or name == 'NBCSNHD':
	                    if Addon.get_setting('free_package') == 'true':
	                        if name in ['CW','ABC','FOX','PBS','CBS','NBC','MY9']:
	                            now.append({
	                                    'name': name,
	...
	if token_check != Addon.get_setting('email'):
	        ...
	        self.passkey = self._get_passkey()
                stream_check = self._get_json('stream/1/live/view', {'token': self.token, 'key': self.passkey, 'scode': 'whtm'})['domain']
            except:
	        self.token = self._login()
    ...
    def _login(self):
        ...
        opener.addheaders = [('User-agent', self.randomagent())]
